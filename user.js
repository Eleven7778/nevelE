...
exports.run = (client, message, args) => {
    ...
    if(!args[0]){
        ...
        .setDescription(`${client.emojis.find(e => e.name === "arrow")} Tag: **${author.tag}**\n${client.emojis.find(e => e.name === "arrow")} Id: **${author.id}**\n${client.emojis.find(e => e.name === "arrow")} Conta criada em: **${moment(author.createdAt).format("LL")}** **(${check(author.createdAt)})**\n${client.emojis.find(e => e.name === "arrow")} Status: **${trad[status]}**\n${client.emojis.find(e => e.name === "arrow")} Jogando: **${playing}**\n${client.emojis.find(e => e.name === "arrow")} Cargos: ${message.member.roles.map(r => `**${r}**`).join(", ")}\n${client.emojis.find(e => e.name === "arrow")} Permissões: ${message.member.permissions.toArray().map(perms => `**${tradd[perms]}**`).join(", ")}`)
        message.channel.send(e)
    } else {
        let member = message.guild.members.find(c => c.name === args.join(" "))
        if(!member)
        return message.channel.send(`${client.emojis.find(e => e.name === "cancel2")} \`${message.author.tag}\`, Não consegui achar este usuário!`)
        ...
        .setColor("36393e")
        .setDescription(`${client.emojis.find(e => e.name === "arrow")} Tag: **${member.user.tag}**\n${client.emojis.find(e => e.name === "arrow")} Id: **${member.user.id}**\n${client.emojis.find(e => e.name === "arrow")} Conta criada em: **${moment(member.user.createdAt).format("LL")}** **(${check(member.user.createdAt)})**\n${client.emojis.find(e => e.name === "arrow")} Status: **${trad[status]}**\n${client.emojis.find(e => e.name === "arrow")} Jogando: **${playing}**\n${client.emojis.find(e => e.name === "arrow")} Cargos: ${member.roles.map(r => `**${r}**`).join(", ")}\n${client.emojis.find(e => e.name === "arrow")} Permissões: ${member.permissions.toArray().map(perms => `**${tradd[perms]}**`).join(", ")}`)
